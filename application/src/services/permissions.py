...
def init_default_roles(db, config, permissions=None):
    if permissions is None:
        permissions = ['base_content', 'likes', 'comments']

    for role_i in config.BASE_ROLES:
        role = Role.query.filter_by(role_name=role_i).first()

        if not role:
            default_role = ((uuid4()), role_i)
